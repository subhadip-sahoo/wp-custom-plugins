    ...
    function populate_related_field_callback(){
        ...
        if(!empty($get_field_ids)){
            foreach ($get_field_ids as $get_field_id) {
                ...
                $all_dependent_ids[] = $get_field_id->id_dependent_field_option;
            }
            $ids = implode(',', $all_ids);
        }else{
            $ids = 'NULL';
...
//            $common_field_ids = 'NULL';
        ...
        foreach ($all_dependent_ids as $id) {
            $check_dependencies = $wpdb->get_results("SELECT id_field_related_to FROM $tbl_wp_relational_fields WHERE id_dependent_field_option = $id");
             if(!empty($check_dependencies)){
                ...
                $other_dependent_field_ids = implode(',', array_unique($dependency_ids));
             }else {
                 $other_dependent_field_ids = 'NULL';
             }
        }
        
        $get_all_common_fields = $wpdb->get_results("SELECT id_dependent_field_option FROM $tbl_wp_relational_fields WHERE id_field_related_to IN($other_dependent_field_ids)", ARRAY_A);
        if(!empty($get_all_common_fields)){
            foreach ($get_all_common_fields as $get_all_common_field) {
                $arr_val[] = $get_all_common_field['id_dependent_field_option'];
            }
            $all_commpn_field_ids[] = array_intersect($all_dependent_ids, $arr_val);
//            $common_field_ids = implode(',', array_intersect($all_dependent_ids, $arr_val));
            foreach (array_intersect($all_dependent_ids, $arr_val) as $common_field) {
                $get_field = $wpdb->get_results("SELECT id_form_field FROM $table_name_options WHERE id_form_field_detail = $common_field");
                foreach ($get_field as $field) {
                    $intersect_field[] = $field->id_form_field;
                }
            }
            $common_field_ids = implode(',', $intersect_field);
        }else{
            $common_field_ids = 'NULL';
        }
